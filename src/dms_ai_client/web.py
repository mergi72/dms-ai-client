...
fileInput.onchange=()=>{const f=fileInput.files[0];if(!f)return clearAttachment();if(f.size>__MAX_ATTACHMENT_BYTES__){status.textContent=`Soubor je větší než ${(__MAX_ATTACHMENT_BYTES__/1048576).toFixed(0)} MB.`;status.className='status error';return clearAttachment()}const reader=new FileReader();reader.onload=()=>{attachment={name:f.name,mime_type:f.type||'application/octet-stream',data_base64:String(reader.result).split(',',2)[1]};attachmentName.textContent=`${f.name} · ${(f.size/1024).toFixed(1)} kB · v kontextu`;removeAttachment.hidden=false;status.textContent='Příloha zůstane v kontextu až do odebrání.';status.className='status'};reader.readAsDataURL(f)};
async function submit(source='keyboard'){let text=input.value.trim();if(!text&&attachment)text='Prohlédni přiložený soubor.';if(!text||send.disabled)return;const sentAttachment=attachment;history.push({role:'user',content:text,attachment:sentAttachment});message('user',text+(sentAttachment?`\n📎 ${sentAttachment.name}`:''),source);input.value='';send.disabled=true;status.textContent='AI přemýšlí a může použít MCP…';status.className='status';try{const r=await fetch('/api/chat',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({messages:history})});const data=await r.json();if(!r.ok)throw new Error(data.error||'Request failed');if(sentAttachment)clearAttachment();traces(data.tool_calls);history.push({role:'assistant',content:data.text});message('assistant',data.text);DMSVoice.speak(data.text);status.textContent=`Hotovo · ${data.model}`;}catch(e){status.textContent=String(e);status.className='status error'}finally{send.disabled=false;input.focus()}}
