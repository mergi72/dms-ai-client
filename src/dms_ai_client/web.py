...
<div id="chat" class="chat"></div><div id="status" class="status">Připraveno</div><div class="voice"><button id="microphone">🎙️ Diktovat</button><button id="speaker">🔇 Čtení vypnuto</button></div><div class="attachments"><button id="attach">📎 Přiložit soubor</button><input id="file" type="file" hidden><span id="attachmentName" class="attachment-name"></span><button id="removeAttachment" hidden>Odebrat</button><label title="Příloha bude součástí každého dalšího API požadavku"><input id="keepAttachment" type="checkbox"> Ponechat pro další dotazy</label><label title="Obsah dokumentu bude předán nastavenému AI providerovi"><input id="allowDocumentContent" type="checkbox"> Povolit Demi číst obsah DMS dokumentů</label></div><div class="composer"><textarea id="input" placeholder="Např. Vypiš dostupná DMS připojení"></textarea><button id="send">Odeslat</button></div>
<script src="/voice.js"></script>
<script>
const chat=document.getElementById('chat'),input=document.getElementById('input'),send=document.getElementById('send'),status=document.getElementById('status'),fileInput=document.getElementById('file'),attachmentName=document.getElementById('attachmentName'),removeAttachment=document.getElementById('removeAttachment'),keepAttachment=document.getElementById('keepAttachment'),allowDocumentContent=document.getElementById('allowDocumentContent');const history=[];let attachment=null;
