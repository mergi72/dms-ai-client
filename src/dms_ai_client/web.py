...
<div id="chat" class="chat"></div><div id="status" class="status">Připraveno</div><div class="voice"><button id="microphone">🎙️ Diktovat</button><button id="send">Odeslat</button><button id="speaker">🔇 Čtení vypnuto</button></div><div class="attachments"><button id="attach">📎 Přiložit soubor</button><input id="file" type="file" hidden><span id="attachmentName" class="attachment-name"></span><button id="removeAttachment" hidden>Odebrat</button><label title="Příloha bude součástí každého dalšího API požadavku"><input id="keepAttachment" type="checkbox"> Ponechat pro další dotazy</label><label title="Obsah dokumentu bude předán nastavenému AI providerovi"><input id="allowDocumentContent" type="checkbox"> Povolit Demi číst obsah DMS dokumentů</label></div><div class="composer"><textarea id="input" placeholder="Např. Vypiš dostupná DMS připojení"></textarea></div>
...
async function submit(source=input.dataset.source||'keyboard'){let text=input.value.trim();if(!text&&attachment)text='Prohlédni přiložený soubor.';if(!text||send.disabled)return;const sentAttachment=attachment,keep=keepAttachment.checked;history.push({role:'user',content:text,attachment:sentAttachment});message('user',text+(sentAttachment?`\n📎 ${sentAttachment.name}`:''),source);input.value='';input.dataset.source='';send.disabled=true;status.textContent='AI přemýšlí a může použít MCP…';status.className='status';try{const r=await fetch('/api/chat',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({messages:history,allow_document_content:allowDocumentContent.checked})});const data=await r.json();if(!r.ok)throw new Error(data.error||'Request failed');if(sentAttachment&&!keep)delete history[history.length-1].attachment;if(sentAttachment)clearAttachment();traces(data.tool_calls);history.push({role:'assistant',content:data.text});const rendered=message('assistant',data.text);DMSVoice.speak(rendered.innerText);status.textContent=`Hotovo · ${data.model}${sentAttachment&&keep?' · příloha zůstává v kontextu':''}`;}catch(e){status.textContent=String(e);status.className='status error'}finally{send.disabled=false;input.focus()}}
...
DMSVoice.initialize(input,document.getElementById('microphone'),document.getElementById('speaker'),status,transcribe,__ASSISTANT_VOICE__);
