...
async function submit(source='keyboard'){let text=input.value.trim();if(!text&&attachment)text='Prohlédni přiložený soubor.';if(!text||send.disabled)return;const sentAttachment=attachment,keep=keepAttachment.checked;history.push({role:'user',content:text,attachment:sentAttachment});message('user',text+(sentAttachment?`\n📎 ${sentAttachment.name}`:''),source);input.value='';send.disabled=true;status.textContent='AI přemýšlí a může použít MCP…';status.className='status';try{const r=await fetch('/api/chat',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({messages:history})});const data=await r.json();if(!r.ok)throw new Error(data.error||'Request failed');if(sentAttachment&&!keep)delete history[history.length-1].attachment;if(sentAttachment)clearAttachment();traces(data.tool_calls);history.push({role:'assistant',content:data.text});message('assistant',data.text);DMSVoice.speak(data.text);status.textContent=`Hotovo · ${data.model}${sentAttachment&&keep?' · příloha zůstává v kontextu':''}`;}catch(e){status.textContent=String(e);status.className='status error'}finally{send.disabled=false;input.focus()}}
